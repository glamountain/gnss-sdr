...
kf_tracking_sptr kf_make_tracking(const Kalman_Conf &conf_)
{
...
kf_tracking::kf_tracking(const Kalman_Conf &conf_) : gr::block("kf_tracking", gr::io_signature::make(1, 1, sizeof(gr_complex)),
                                                                              gr::io_signature::make(1, 1, sizeof(Gnss_Synchro)))
{
    ...
    d_carrier_kalman_filter = Tracking_Kalman_filter(static_cast<float>(d_code_period), trk_parameters.kf_order, trk_parameters.kf_extended);
...
void kf_tracking::start_tracking()
{
    ...
    d_carrier_loop_filter.initialize();    // initialize the carrier loop filter
    d_carrier_kalman_filter.initialize();  // initialize the carrier kalman filter
    d_code_loop_filter.initialize();       // initialize the code loop filter
    ...
    d_carrier_kalman_filter.set_pdi(static_cast<float>(d_code_period));
...
bool kf_tracking::cn0_and_tracking_lock_status(double coh_integration_time_s)
{
    ...
    if (d_cn0_estimation_counter < trk_parameters.cn0_samples)
    ...
    else
        {
            d_cn0_estimation_counter = 0;
            // Code lock indicator
            d_carrier_power_snv = cn0_svn_estimator(d_Prompt_buffer, trk_parameters.cn0_samples, coh_integration_time_s);
...
void kf_tracking::run_filters()
{
    ...
    // ################## KF ###########################################################
    else
        {
            ...
            if (trk_parameters.kf_extended)
                {
                    // Carrier correlator output filter
                    carr_nco d_carrier_nco = d_carrier_kalman_filter.get_carrier_nco(*d_Prompt_Data, d_carrier_power_snv);
                ...
                {
                    // Update discriminator noise estimation
                    d_carrier_kalman_filter.set_KF_R(d_CN0_SNV_dB_Hz);

                    // Carrier correlator output filter
                    carr_nco d_carrier_nco = d_carrier_kalman_filter.get_carrier_nco(d_carr_error_hz, trk_parameters.pll_bw_hz);

                    // New carrier Doppler frequency estimation
                    d_carrier_doppler_hz = d_carrier_nco.second;
                    d_carr_error_filt_hz = d_carrier_nco.first / PI_2;
                }
        }
...
void kf_tracking::log_data(bool integrating)
{
    if (trk_parameters.dump)
        {
...
int32_t kf_tracking::save_matfile()
{
    // READ DUMP FILE
        ...
        case 1:  // Pull-in
            {
                ...
                if (trk_parameters.kf_order == 3U)
                    {
                        d_carrier_kalman_filter.set_KF_x_ini(d_carrier_phase_step_rad * samples_offset, d_carrier_doppler_hz, 0.0);
                    }
                else
                    {
                        d_carrier_kalman_filter.set_KF_x_ini(d_carrier_phase_step_rad * samples_offset, d_carrier_doppler_hz);
                        //d_carrier_kalman_filter.set_KF_x_ini(d_acc_carrier_phase_rad, d_carrier_doppler_hz);
                    }
                d_carrier_kalman_filter.initialize();
                                ...
                                if (d_enable_extended_integration)
                                    {
                                        ...
                                        d_carrier_kalman_filter.set_pdi(new_correlation_time);
