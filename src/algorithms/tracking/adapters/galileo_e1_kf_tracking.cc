/*!
 * \file galileo_e1_kf_tracking.cc
...
#include "kalman_conf.h"
#include "galileo_e1_kf_tracking.h"
...
GalileoE1KfTracking::GalileoE1KfTracking(
    ConfigurationInterface* configuration, std::string role,
    unsigned int in_streams, unsigned int out_streams) : role_(role), in_streams_(in_streams), out_streams_(out_streams)
{
    Kalman_Conf trk_param = Kalman_Conf();
    DLOG(INFO) << "role " << role;
    ...
    if (item_type.compare("gr_complex") == 0)
        {
            ...
            tracking_ = kf_make_tracking(trk_param);
        }
    else
        {
            item_size_ = sizeof(gr_complex);
...
void GalileoE1KfTracking::start_tracking()
{
 ...
 */
void GalileoE1KfTracking::set_channel(unsigned int channel)
{
    channel_ = channel;
...
void GalileoE1KfTracking::set_gnss_synchro(Gnss_Synchro* p_gnss_synchro)
{
    tracking_->set_gnss_synchro(p_gnss_synchro);
}


void GalileoE1KfTracking::connect(gr::top_block_sptr top_block)
{
    if (top_block)
...
void GalileoE1KfTracking::disconnect(gr::top_block_sptr top_block)
{
    if (top_block)
        { /* top_block is not null */
        };
...
gr::basic_block_sptr GalileoE1KfTracking::get_left_block()
{
    return tracking_;
}


gr::basic_block_sptr GalileoE1KfTracking::get_right_block()
{
    return tracking_;
}
